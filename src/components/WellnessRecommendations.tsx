...
const recommendations = [
  ...
  },
  {
    id: 6,
    title: "Calming Music",
    description: "Listen to nature sounds or instrumental music to reduce stress levels.",
    category: "Relaxation",
    duration: "25 min",
    icon: Music,
    color: "bg-wellness-calm",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {recommendations.map((rec) => (
            <Card 
              key={rec.id} 
              className={`
                gradient-card border-0 shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl
                ${rec.urgent ? 'ring-2 ring-accent ring-opacity-50' : ''}
              `}
            >
              <CardHeader className="pb-4">
                <div className="flex items-start justify-between">
                  <div className={`p-3 rounded-xl ${rec.color} bg-opacity-20`}>
